package
{
   ...
   public class NewPipBoyShared
   {
      ...
      public static function GetProvidersForPageTab(param1:uint, param2:uint) : Vector.<String>
      {
            ...
            case STATS_PAGE:
               switch(param2)
               {
                  case STATS_TAB_STATUS:
                     _loc3_ = ALL_PROVIDERS[STATS_PAGE].slice(0,1);
                     break;
                  case STATS_TAB_EFFECTS:
                  case STATS_TAB_PERKS:
                     _loc3_ = ALL_PROVIDERS[STATS_PAGE].slice(3,4);
                     break;
                  case STATS_TAB_SPECIAL:
                     _loc3_ = ALL_PROVIDERS[STATS_PAGE].slice(4,5);
                     break;
                  case STATS_TAB_COLLECTIONS:
                     _loc3_ = ALL_PROVIDERS[STATS_PAGE].slice(5,6);
                     break;
                  case STATS_TAB_PREFIX:
                  case STATS_TAB_SUFFIX:
                     _loc3_ = ALL_PROVIDERS[STATS_PAGE].slice(6,7);
               }
               break;
            case INV_PAGE:
               _loc3_ = ALL_PROVIDERS[INV_PAGE].slice(0,2);
               break;
            case DATA_PAGE:
               switch(param2)
               {
                  ...
                  case DATA_TAB_DAILY:
                     _loc3_ = ALL_PROVIDERS[DATA_PAGE].slice(0,1);
                     break;
                  case DATA_TAB_LEADS:
                  case DATA_TAB_MISC:
                     _loc3_ = ALL_PROVIDERS[DATA_PAGE].slice(1,2);
               }
               break;
            case RADIO_PAGE:
               _loc3_ = ALL_PROVIDERS[RADIO_PAGE].slice(0,1);
         }
         return _loc3_;
      }
   }
