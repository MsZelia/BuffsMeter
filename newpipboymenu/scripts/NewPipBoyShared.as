package
{
   ...
   public class NewPipBoyShared
   {
      ...
      public static function GetProviderForPageTab(param1:uint, param2:*) : String
      {
         var _loc3_:String = "";
         switch(param1)
         {
            case STATS_PAGE:
               switch(param2)
               {
                  case STATS_TAB_STATUS:
                     _loc3_ = ALL_PROVIDERS[STATS_PAGE][0];
                     break;
                  case STATS_TAB_EFFECTS:
                  case STATS_TAB_PERKS:
                     _loc3_ = ALL_PROVIDERS[STATS_PAGE][1];
                     break;
                  case STATS_TAB_SPECIAL:
                     _loc3_ = ALL_PROVIDERS[STATS_PAGE][2];
                     break;
                  case STATS_TAB_COLLECTIONS:
                     _loc3_ = ALL_PROVIDERS[STATS_PAGE][3];
                     break;
                  case STATS_TAB_PREFIX:
                  case STATS_TAB_SUFFIX:
                     _loc3_ = ALL_PROVIDERS[STATS_PAGE][4];
               }
               break;
            case INV_PAGE:
               _loc3_ = ALL_PROVIDERS[INV_PAGE][0];
               break;
            case DATA_PAGE:
               switch(param2)
               {
                  ...
                  case DATA_TAB_DAILY:
                     _loc3_ = ALL_PROVIDERS[DATA_PAGE][0];
                     break;
                  case DATA_TAB_LEADS:
                  case DATA_TAB_MISC:
                     _loc3_ = ALL_PROVIDERS[DATA_PAGE][1];
               }
               break;
            case RADIO_PAGE:
               _loc3_ = ALL_PROVIDERS[RADIO_PAGE][0];
         }
         return _loc3_;
      }
   }
